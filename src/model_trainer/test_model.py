...
from config import MODEL_NAME, MODEL_SAVE_DIR, BASE_DIR, MODEL_VERSION
...
def test_single_file(data_dir, model_path, result_dir):
    ...
    # 加载模型
    model = SleepStageClassifier(input_size=5).to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()

    # 预测睡眠阶段
    predictions = predict_sleep_stages(model, df["timestamp"].values, df["heart_rate"].values, device)
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=["Wake", "Light", "Deep", "REM"], yticklabels=["Wake", "Light", "Deep", "REM"])
    ...
    # 生成分类报告
    report = classification_report(df["sleep_stage"], df["predicted_stage"], target_names=["Wake", "Light", "Deep", "REM"])
    ...
    for i in range(0, len(df) - window_size, window_size // 2):
        window_accuracy = (df["sleep_stage"].iloc[i : i + window_size] == df["predicted_stage"].iloc[i : i + window_size]).mean()
